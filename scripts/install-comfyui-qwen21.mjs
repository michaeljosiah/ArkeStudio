...
const source = fileURLToPath(new URL("../vendor/comfyui/ArkeQwen21Runtime/", import.meta.url));
const manifest = JSON.parse(await readFile(join(source, "manifest.json"), "utf8"));
const hash = (bytes) => createHash("sha256").update(bytes).digest("hex");

/** Never bless or overwrite a different user checkout. Verify the complete Python module first. */
export async function installQwen21Runtime(engineDir) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv.length !== 3) throw new Error("Usage: node scripts/install-comfyui-qwen21.mjs <ComfyUI directory>");
  const target = await installQwen21Runtime(process.argv[2]);
  console.log(`Verified Qwen 2.1 runtime guard: ${target}\nRestart ComfyUI if the node was not already loaded, then re-verify in Arke.`);
}
