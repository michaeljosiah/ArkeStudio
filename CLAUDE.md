...
- **Remote browsers:** the coordinator stays on loopback. Another device reaches it only through
  a TLS proxy (Tailscale Serve) named by a `wss:` `VITE_ARKE_WS` plus `ARKE_DEV_LOCAL_WS`. Do
  not add a non-loopback bind or accept a remote `ws:` endpoint: either puts the capability on
  the wire. The dev plugin widens Vite's allowed hosts and the page policy for that one declared
  origin and server only; the built page keeps its loopback-only policy.
