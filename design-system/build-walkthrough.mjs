...
const STEPS = [
  ...
  },
  {
    ...
    actions: [{ match: "Accept", to: "101c", hint: "the same moment, in turn 101's words" }],
    built: { status: "missing", route: "nothing ships this yet",
      notes: ["<b>Arke's signature interaction, drawn (turn 100): the screen shows what the conversation is making, while it makes it.</b> Two scenes appear <b>in their place</b> between 03 and the bell, marked <code>NEW · NOT WRITTEN YET</code>; the promise line the conversation changed shows its new words marked <code>STAGED</code>; and the two settled scenes after them renumber, because that is what inserting does.",
              "<b>Turn 101 re-says the panel's half of this in plain words</b> &#8212; <i>I&#8217;ve shaped this into five scenes</i>, the things themselves, <code>Apply changes</code> / <code>Keep discussing</code> &#8212; and <b>101c is the frame to read for that</b>. This one still says <code>2 scenes · 1 promise line</code> under Accept / Discard, which is the count-and-yes turn 100 drew and turn 101 replaced.",
              ...
              "<b>The instinct already shipped once:</b> turn 92's <code>STAGED · NOT WRITTEN YET</code> tile on the season board does exactly this for one case, and it is the only place in the app that does. Everywhere else a change is a list of fields on another screen.",
              "<b>Nothing here is built.</b> What exists to build on: proposals are already durable objects with a gate that accepts them (<code>.proposals/pr_*</code>), so what is missing is the drawing of one, not the machinery."] },
  },
  {
    id: "101c", step: "A change, in plain words",
    actions: [{ match: "Apply changes", to: "14c", hint: "apply, and open a scene" }],
    built: { status: "missing", route: "nothing ships this yet",
      notes: ["<b>The governance vocabulary comes off the screen (turn 101).</b> <code>Ready to accept</code>, <code>Accept Proposal</code>, <code>staged</code>, <code>Discard</code> &#8212; every one of them is exactly right underneath and none of them is a thing a person wants to read. Arke says what it did in one sentence, <b>the things it made are listed as themselves</b>, and there are two buttons.",
              "<b><code>Keep discussing</code> is not <code>Discard</code>.</b> The old pair offered yes or destroy it, so a person who wanted to say <i>almost &#8212; make three of them shorter</i> had to throw the work away to ask. Keep discussing leaves the change standing and returns to the composer.",
              "<b>The before-and-after is promoted, not deleted.</b> A field-by-field diff is what you show when a change would overwrite or delete something a person wrote; for anything additive it is a form standing between somebody and their own work. 91b's card survives as the dangerous case.",
              "<b>What ships:</b> the machinery, whole and unchanged &#8212; <code>.proposals/pr_*</code>, the gate, versions, <code>.history/</code> &#8212; and the accept itself, verified. <b>What does not:</b> every word on this frame. The panel today says <code>Ready to accept</code> over a list of fields with <code>Accept Proposal</code> beneath it.",
              "The five cards on the page are turn 100's staged-in-place, also unbuilt: this frame needs both halves to be true at once."] },
  },
  {
    ...
    built: { status: "built", route: "#/w/:worldId/p/:prodId/scenes/:sceneId",
      notes: ["<b>Newly adopted (turn 97), replacing 14a.</b> The scene page is its storyboard and the storyboard is the editor: the script is written on the card itself, <code>@</code> names anything in the world and rides along as an image reference, and <code>Advanced</code> (under a card's <code>⋯</code> — scroll the strip to shot 15) opens the full shot.",
              ...
              "<b>A hand edit saves where it stands</b> — the bible's model: versioned, <code>saved 2 minutes ago</code>, <code>version history</code>, a save against a moved base refused. <b>An assistant edit asks first</b>: plan cards in the docked panel &#8212; drawn as <code>Apply to shots</code> / <code>Discard</code>, which turn 101 re-says as <code>Apply changes</code> / <code>Keep discussing</code>, the same sentence one level down, and an invented shot stays <code>suggested</code> until confirmed. Scene Chat's separate screen (turn 94) is amended into this panel.",
              ...
              "<b>Turn 135 (built 2026-09-10, SPEC-044, epic 1066):</b> the cast row under the context chips with its dashed door and picker, the character dialog (Look, Voice, Lines in this scene; Record a line and Generate a line as sheets; Keep selects), the place chip and the location dialog, the band chip's <code>voice · look</code>, the Flow's character card with <code>Open</code>, the plan card's per-pass line from the coordinator's own summary, <code>Play lines</code> on Preview in place of the table read, and full screen for Flow and Stage. What left the page — dialogue guidance, master playback, the per-dispatch performance picker, the table-read panel with its notes, the per-shot performance panel — has no surface now; the audio foundation's handlers and records stay."] },
  },
  {
    ...
    built: { status: "built", route: "#/w/:worldId/p/:prodId/scenes/:sceneId/shots/:shotId",
      notes: ["<b>The full shot behind the card (turn 97):</b> script, the assembled prompt with <code>Rebuild</code>, cinematic intent — framing, lens and movement inferred from it, anything set by hand wins — <b>Timing</b> (turn 101: a beat is a word from the craft, and a section heading should be the plainest true word; the drawing still says <i>timing beats</i>), references, and Creative / Camera / Sound / Continuity / Technical.",
