...
import { mkdir, readFile, writeFile, link, copyFile, stat, rename } from "node:fs/promises";
...
if (!engineDir || !modelsDir || !intakeDir || !outputDir) throw new Error("Usage: smoke-h3-adapters.ts <engine directory> <models directory> <verified intake directory> <output directory> [recipe ID] [comma-separated adapter indices|all]");
...
const indices = entry.split(",");
if (entry !== "all" && indices.some(index => !/^\d+$/.test(index))) throw new Error("Invalid adapter index");
const releases = entry === "all" ? HEARMEMAN_ADAPTERS : [...new Set(indices.map(Number))].map(index => HEARMEMAN_ADAPTERS[index]);
...
// Bound every HTTP operation, including poll and cancellation, whose provider methods do not
// take the job signal. A silent engine must not prevent the harness from writing its outcome.
const boundedFetch: typeof fetch = (url, init) => fetch(url, { ...init,
  signal: AbortSignal.any([...(init?.signal ? [init.signal] : []), AbortSignal.timeout(30_000)]) });
async function saveReport(directory: string, report: Record<string, unknown>): Promise<void> {
...
const system = await (await boundedFetch(`${endpoint}/system_stats`)).json();
const { stdout: engineCommit } = await run("git", ["rev-parse", "HEAD"], { cwd: engineDir, windowsHide: true });
for (const release of releases) {
  const deadline = AbortSignal.timeout(90 * 60_000);
  const directory = join(output, release!.id);
  await mkdir(directory, { recursive: true });
  // Preserve interrupted attempts before an explicitly selected rerun replaces the live report.
  await copyFile(join(directory, "report.json"), join(directory, `report-before-${Date.now()}.json`), constants.COPYFILE_EXCL)
  ...
  try {
    ...
    for (;;) {
      ...
      await saveReport(directory, report);
      let result;
      try { result = await client.poll("", remoteId); }
      catch (error) {
        // A temporarily unresponsive engine is not evidence that generation failed. Reconcile
        // the same prompt until the bounded job deadline; never submit another copy on timeout.
        report.lastPollError = error instanceof Error ? error.message : String(error);
        await saveReport(directory, report);
        console.log(`${release!.displayName}: poll unavailable; retaining ${remoteId}`);
  ...
  // An adapter-specific failure should not hide results for later candidates. Continue only
  // after confirming the engine is idle; uncertain cancellation must stop the batch.
  if (report.status === "failed" && !await idle().then(() => true, () => false)) break;
}
