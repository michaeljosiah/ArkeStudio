...
function sidecarBoundary(files: ReadonlyArray<{ path: string; newContent?: string | null }>): number {
  ...
  for (const file of files) {
    ...
    try {
      const record = JSON.parse(file.newContent) as { mediaInfo?: Record<string, unknown>; retiredAt?: unknown; generation?: { source?: unknown; workflow?: unknown; directionHash?: unknown; remakeOf?: unknown } } | null;
...
export class Committer {
  ...
  async commit(input: CommitInput, hooks: CommitHooks = {}): Promise<CommitResult> {
    ...
    const landsStagePerformance = files.some(
      (f) => classify(f.path).track === "scene" && f.newContent != null && carriesStagePerformance(f.newContent),
    );
