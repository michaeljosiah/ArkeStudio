 ...
 */
export function verifyCandidates(raw: RawCandidate[], sourceText: string, alreadyDecided: string[]): VerifiedBatch {
  ...
  if (decision === "accept") {
    if (candidate.kind === "canon") {
      ...
      const staged = await gate.stage({
        kind: "new-sheet",
        summary: `Extracted ${kind}: ${candidate.name} (from ${artifact.file})`,
