...
describe("filing (R-1, R-4, D8, D9, §3.2)", () => {
  it("retires durably without erasing bytes or provenance, and re-import restores the same identity (#957)", async () => {
    ...
    try {
      ...
      assert.equal((await readWorldMeta(dir)).schemaVersion, 18);
      await assert.rejects(readWorldMeta(dir, { supports: 17 }), /newer|schema/i);
      await retireArtifact(store, artifact.id);
      assert.equal(await readFile(path, "utf8"), raw, "repeated retirement does not rewrite history");
      await storeBatch(store, artifact, { verified: [], droppedCount: 1, droppedReasons: ["unverified"] });
      assert.equal(store.getBundle().artifacts.find(a => a.id === artifact.id)?.retiredAt, retired.retiredAt, "late extraction preserves retirement");
  ...
  it("restores the retired filename ahead of other retired and active occurrences of identical bytes (#957)", async () => {
    const { store, dir } = await open();
    try {
      const sourcePath = await sourceFile("restore-this.txt", "same generated bytes");
      const filed = await fileArtifact(store, { sourcePath });
      assert.equal(filed.outcome, "filed");
      if (filed.outcome !== "filed") return;
      const artifact = filed.artifact;
      await store.gateOp(async () => {
        for (const [file, retiredAt] of [["aaa-active.txt", undefined], ["aaa-retired.txt", CLOCK()]] as const) {
          await writeFile(join(dir, "artifacts", file), "same generated bytes");
          await writeFile(join(dir, "artifacts", `${file}.json`), JSON.stringify({ ...artifact, id: newId("ar"), file, retiredAt, origin: { by: "system", producedBy: file } }));
        }
      });
      await retireArtifact(store, artifact.id);
      const restored = await fileArtifact(store, { sourcePath });
      assert.equal(restored.outcome, "deduplicated");
      if (restored.outcome === "deduplicated") assert.equal(restored.artifact.id, artifact.id);
      assert.ok(store.getBundle().artifacts.find(a => a.file === "aaa-retired.txt")?.retiredAt);
    } finally { await store.close(); }
  });
