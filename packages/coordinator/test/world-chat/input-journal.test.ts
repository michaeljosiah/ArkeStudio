...
import {
  newId, WorldChatInputRequestSchema, WORLD_CHAT_INPUT_SCHEMA_VERSION,
...
describe("durable additional conversation inputs (SPEC-045)", () => {
  ...
  it("promotes FIFO with the original identity and constraints in the same event as the primary run", async () => {
    ...
    await assert.rejects(journal.promote(tail!.input.messageId, second.queue.revision, run(), ROUTING, "skip"), /oldest/);
    const primary = run();
    ...
    await assert.rejects(journal.promote(tail!.input.messageId, promoted.queue.revision, run(), ROUTING, "overlap"), /still active/);
  });
  ...
  it("serializes promotion against Stop without losing the waiting input", async () => {
    ...
    const outcomes = await Promise.allSettled([
      journal.pause("stopped", "stop"), other().promote(messageId, queued.queue.revision, run(), ROUTING, "start"),
    ]);
  ...
  it("requires explicit Continue to approve a changed route", async () => {
    ...
    const promoted = await journal.promote(messageId, resumed.queue.revision, primary, routing, "reviewed-route");
  ...
  it("captures attachments and constraints immutably and refuses changed attachments at promotion", async () => {
    ...
    await assert.rejects(journal.promote(row.input.messageId, receipt.queue.revision, run(), ROUTING, "changed-attachment"), /attachment changed/);
