...
import { ModelWorldChatActionSchema, changedSpan, newId, passageDiff, type ConversationId } from "@arke-studio/contracts";
 ...
 * the chapter moves. Kept in this file rather than one of its own, because a new coordinator test
 * file reshuffles `--test-shard` and moved Windows process suites into the shard whose job then
 * runs the adapters' timing-sensitive tests (PR 1232).
 */
...
describe("keeping part of a passage revision (turn 128)", () => {
  it("replaces only the span with the edits kept, restamps the words and moves the draft revision", async () => {
    const { gate, stage, read } = await openKeep();
  ...
  it("refuses a whole-chapter draft, and a passage whose chapter moved after it was staged", async () => {
    const { dir, gate, stage, read } = await openKeep();
    const live = await read(CHAPTER);
