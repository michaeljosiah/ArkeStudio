...
describe("cold-scan budget (§2.13)", () => {
  it("scans a 50-sheet, 200-entry, 500-take world in under ten seconds", async (t) => {
     ...
     * different: measured on GitHub's runners, the same corpus and the same code write in 53–60ms
     * on ubuntu-latest and 563–2112ms on windows-latest, against scans of 203–260ms and 303–518ms.
     * The scan barely notices which one it is on. The ratio moves by a factor of ten — about 4x on
     * Linux, about 0.5x on Windows — because the denominator is reporting the cost of creating a
     * file on NTFS, not the speed of the machine.
     *
     * Which made a budget of 8x a ceiling of roughly 450ms on Linux, and somewhere between 4.5 and
     * 17 seconds on Windows: the ten-second obligation rewritten as half a second on the runner
     * least able to keep it, and left vacuous on the platform we ship. One descheduled scan spends
     * the whole Linux margin, and one did — a branch whose only diff was six markdown files went
     * red at 8.9x on a 544ms scan. The figure is still worth reading, because a scan that suddenly
     * costs many times its own corpus write is worth a look, but it is a reading and not a gate.
     */
    t.diagnostic(
