...
export function GenesisReadinessCard({ review, busy, onRefresh, onFix, onLeave }: {
  ...
  useEffect(() => { pendingRef.current = false; setPending(false); }, [review]);
  return <section aria-label="World readiness review">
    ...
    {review && <>
      ...
      {review.findings.map(finding => <Callout key={finding.id} title={finding.title}>
        ...
        {!finding.leftOpen && !["blocker", "approval"].includes(finding.category) && <Button variant="ghost" disabled={busy || pending} onClick={() => {
          if (pendingRef.current) return;
          pendingRef.current = true; setPending(true); onLeave(finding.id, review.digest);
...
export function FoundingProgressCard({ build }: { build: FoundingBuildState }) {
  ...
  if (build.status !== "running" && !build.shortfall && build.items.every(item => item.state === "landed" || !item.authorized)) return null;
  return <section aria-label="Founding progress" className="fy-actioncard">
    ...
    {build.items.filter(item => item.state !== "landed").map(item => <div key={item.key}>
      ...
      {build.status !== "running" && ["failed", "skipped"].includes(item.state) &&
