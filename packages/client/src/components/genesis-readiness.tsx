...
export function FoundingProgressCard({ build }: { build: FoundingBuildState }) {
  const running = build.status === "running" || build.items.some(item => item.state === "running");
  if (build.noticeDismissed && !running) return null;
  if (!running && !build.shortfall && build.items.every(item => item.state === "landed" || !item.authorized)) return null;
  return <section aria-label="Founding progress" className="fy-actioncard">
    <h3>{build.worldName} · {running ? "running" : build.status}</h3>
    ...
    {build.items.filter(item => item.state !== "landed").map(item => <div key={item.key}>
      ...
      {item.state === "held" && <Button onClick={() => openActivityPanel("inbox")}>Resolve in Activity</Button>}
      {!running && ["failed", "skipped"].includes(item.state) && (["world", "author-sheet", "thread", "prop", "selected-image", "selected-voice"].includes(item.kind) ?
        <Button onClick={() => runBuildItem(build.worldId, item.key)}>Retry {item.name}</Button>
        : <Button onClick={() => openActivityPanel("inbox")}>Review retry in Activity</Button>)}
    </div>)}
    <p>Completed items are kept. Generation retries are new work; review them in Activity.</p>
    {running ? <Button onClick={() => stopFoundingBuild(build.worldId)}>Stop and skip remaining work</Button>
