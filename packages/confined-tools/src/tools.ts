...
import { object, type JsonObject } from "./json.js";

/**
 * The tools a confined session may be offered, and what running one returns (issue 1247).
 *
 * Harness-neutral on purpose: each harness that executes tools itself maps these to its own
 * wire shapes — Codex's dynamic functions and input items, a local loop's function tools — so
 * the confinement, the file access through the pinned root and the world-query client are one
 * implementation rather than one per harness.
 */
export interface ToolDefinition { name: string; description: string; parameters: JsonObject }
/** Text, or an image as base64 with its type. */
export type ToolContent = { type: "text"; text: string } | { type: "image"; mimeType: string; data: string };
export interface ToolResult { success: boolean; content: ToolContent[] }
export interface ToolSession {
  root: string;
  rootIdentity: FileIdentity;
  confinement: AgentConfinement;
  worldQueryUrl?: string;
  worldTools: Map<string, ToolDefinition>;
  inputModalities?: ("text" | "image")[];
}
export interface ExecutedTool { result: ToolResult; summary?: string }
const MAX_TEXT = 128 * 1024;
const textResult = (text: string): ToolResult => ({ success: true, content: [{ type: "text", text }] });
...
const FILE_TOOLS: ToolDefinition[] = [
  { name: "read", description: "Read a file in this session. PNG, JPEG, GIF and WebP return actual image content. Text reads may use offset and limit.", parameters: schema({ path: string, offset: { type: "integer", minimum: 0 }, limit: { type: "integer", minimum: 1, maximum: MAX_TEXT } }, ["path"]) },
  { name: "list", description: "List one directory in this session. Links are excluded.", parameters: schema({ path: string }, []) },
  { name: "search", description: "Find literal text inside this session's text files. No shell or regular expressions. Results include file names and line numbers.", parameters: schema({ query: string, path: string, limit: { type: "integer", minimum: 1, maximum: 100 } }, ["query"]) },
  { name: "write", description: "Write a UTF-8 text file inside this session. Creates parent directories. Refuses replacement of binary files. Only proposal files may be changed.", parameters: schema({ path: string, content: string }, ["path", "content"]) },
  { name: "edit", description: "Replace exactly one matching passage inside a session's UTF-8 text file. Refuses binary files and zero or multiple matches.", parameters: schema({ path: string, oldText: string, newText: string }, ["path", "oldText", "newText"]) },
];
const INTENTS = { read: "read", list: "list", search: "search", write: "edit", edit: "edit" } as const;

export function toolsFor(session: ToolSession): ToolDefinition[] {
...
export async function discoverWorldTools(session: ToolSession, signal?: AbortSignal): Promise<void> {
  ...
  for (const raw of response.tools) {
    ...
    session.worldTools.set(name, { name, description: tool.description, parameters: object(tool.inputSchema) });
  }
}
...
async function executeOneTool(session: ToolSession, name: string, args: JsonObject, signal: AbortSignal): Promise<ExecutedTool> {
  ...
  const allowed = Object.keys(object(definition.parameters.properties));
...
async function executeFileTool(session: ToolSession, files: ConfinedFiles, name: string, args: JsonObject, signal: AbortSignal): Promise<ExecutedTool> {
  if (name === "read") {
    ...
    if (mime) return { result: { success: true, content: [{ type: "text", text: `Read image ${basename(raw)}.` }, { type: "image", mimeType: mime, data: bytes.toString("base64") }] }, summary: `read ${basename(raw)}` };
