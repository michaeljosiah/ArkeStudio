...
it("packed engine installs and executes with no source or workspace fallback", { timeout: 300000 }, async () => {
  ...
  try {
    ...
    await writeFile(join(root, "consumer.ts"), 'import { createEngine, type EnginePolicy, type EngineContext } from "@arke-studio/engine";\nimport { JobQueue, FsWorldProvider } from "@arke-studio/engine/local";\nconst context: EngineContext = { actorId: "a", scopeId: "s", subjectId: "c", executorId: "w" };\nconst policy = {} as EnginePolicy;\ndeclare const engine: ReturnType<typeof createEngine>;\nasync function proseJourney() {\n  const production = await engine.prose.createProduction(context, "world", { operationId: "production", title: "Story" });\n  const p = production.value.productionId;\n  const chapter = await engine.prose.createChapter(context, "world", p, { operationId: "chapter", title: "Home", order: 1 });\n  const c = chapter.value.chapterId;\n  const read = await engine.prose.readChapter(context, "world", p, c);\n  const saved = await engine.prose.saveChapter(context, "world", p, c, { operationId: "save", body: "Hello", baseHash: read.hash });\n  const version: number = saved.value.version;\n  return version;\n}\nvoid [createEngine, JobQueue, FsWorldProvider, context, policy, proseJourney];\n');
    ...
    await cp(resolve("test/writing-consumer.ts"), join(root, "writing-consumer.ts"));
    await run(process.execPath, [resolve("../../node_modules/typescript/bin/tsc"), "--noEmit", "--strict", "--module", "NodeNext", "--moduleResolution", "NodeNext", "--target", "ES2022", "writing-consumer.ts"], root);
