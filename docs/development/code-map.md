...
| Read aloud | client `src/components/read-aloud.tsx` → contracts `src/prose.ts` and the `read-prose` / `read-sheet-section` / `read-bible-section` frames → coordinator `src/coordinator.ts` (`narrateSection`) and `voice/service.ts` (`authoritativeProseSpeech`, `authoritativeSheetSpeech`, `authoritativeBibleSpeech`). A screen names a source; the coordinator reads the authoritative record and narrates it with the app-level narrator, never a character’s voice | coordinator `test/voice/service.test.ts`; client `test/voice-line.test.tsx`; contracts `test/schemas.test.ts` |
