...
| Chapter workspace (story) | client `src/screens/chapter-workspace.tsx` (open on demand, autosave against the base it read, restore, the staged draft in the prose's place, Arke docked on the production thread), `screens/production.tsx` (`ChapterTreeScreen`, `useNewChapter`); contracts `src/prose.ts` (`chapterParagraphs`, `countWords`, `targetWords`), the `create-chapter` / `open-chapter` / `save-chapter` / `restore-chapter` frames and the `chapter.*` events; coordinator `src/productions/ops.ts` (`createChapter`, `openChapter`, `saveChapter`, `restoreChapter`) and the matching cases in `src/coordinator.ts` | coordinator `test/productions/chapters.test.ts`, `test/voice/chapter-read.test.ts`; client `test/chapter-workspace.test.tsx`, `test/chapter-tree.test.tsx`; [design turn 126](../../design-system/Arke%20Studio.dc.html#t126), issue 874 |
...
Render planning, migration and inspector validation receive the whole world's artifact catalog. Contracts `artifact-access.ts` (`resolveProductionArtifact`) distinguishes missing media from another production's scoped material; the same decision guards timeline/Library placement and legacy overlay writes. Render planning's `legacyArtifactScopeRefusal` also checks unmigrated audio and song masters before preview or export. Picker lists remain scoped to the current production (SPEC-020 R-13).
