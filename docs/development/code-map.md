...
| Stage timeline controls (issues 1040, 1041) | client `screens/scene-workspace/stage.tsx`: pointer scrubbing, local keyboard shortcuts, optional looping and selectable/retimable performance and object lanes; frame stepping uses contracts `STAGE_FRAME_RATE`. All key edits share the existing draft and Keep command. | client `test/scene-stage.test.tsx` |
