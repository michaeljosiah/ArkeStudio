...
| Chapter workspace (story) | client `src/screens/chapter-workspace.tsx` (open on demand, autosave against the base it read, restore, the staged draft or passage in the prose's place, the selection as the dock's subject, Arke docked on the production thread), `screens/production.tsx` (`ChapterTreeScreen`, `useNewChapter`, the style cards on `OverviewStoryScreen`); contracts `src/prose.ts` (`chapterParagraphs`, `countWords`, `targetWords`, `changedSpan`, `passageOf`), `src/world.ts` (`ProseStyleSchema`), the `create-chapter` / `open-chapter` / `save-chapter` / `restore-chapter` / `edit-chapter-plan` frames and the `chapter.*` events; coordinator `src/productions/ops.ts` (`createChapter`, `openChapter`, `saveChapter`, `restoreChapter`, `editChapterPlan`, `overviewSteer`), `src/world-chat/production-authoring.ts` (`production-chapter` with `passage`, applied at staging by `replacePassage`; `production-prose-style` staged to `prose-style.json`, a JSON track of its own), and the matching cases in `src/coordinator.ts` | coordinator `test/productions/chapters.test.ts`, `test/voice/chapter-read.test.ts`, `test/world-chat/chapter-passage.test.ts`, `test/gate/chapter-review.test.ts`; client `test/chapter-workspace.test.tsx`, `test/chapter-tree.test.tsx`, `test/prose-style-overview.test.tsx`; contracts `test/prose.test.ts`; [design turns 126–128](../../design-system/Arke%20Studio.dc.html#t128), issues 874, 882, 896 |
