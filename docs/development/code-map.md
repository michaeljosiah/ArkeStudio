...
Render planning and migration receive the whole world's artifact catalog. Contracts `artifact.ts` (`resolveProductionArtifact`) distinguishes missing media from another production's scoped material; the same decision guards timeline/Library placement and legacy overlay writes. Picker lists remain scoped to the current production (SPEC-020 R-13).
