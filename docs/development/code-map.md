...
Render planning and migration receive the whole world's artifact catalog. Contracts `artifact-access.ts` (`resolveProductionArtifact`) distinguishes missing media from another production's scoped material; the same decision guards timeline/Library placement and legacy overlay writes. Its `legacyArtifactScopeRefusal` also checks unmigrated audio and song masters before preview or export. Picker lists remain scoped to the current production (SPEC-020 R-13).
